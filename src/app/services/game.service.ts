...
import { Card, Colors, DeckService } from './deck.service';
import { wrapToInterval } from 'src/utils';
...
export class GameService {
  ...
  public async startGame(playerCount: number): Promise<void> {
    ...
    for (let i = 1; i <= this.deckService.getHandSize(); i++) {
  ...
  private getNextPlayer(skipOne: boolean = false): string {
    ...
    nextPlayer = wrapToInterval(nextPlayer, 1, this.state.playerCount);
